...
Texture::Texture(SDL_Texture *t, json_t *json) {
  ...
  for (i = 0; i < json_array_size(json); i++) {
    ...
    key = std::string(json_string_value(json_object_get(frame, "filename")));
    coords = json_object_get(frame, "frame");

    xywh.push_back(
        static_cast<int>(json_number_value(json_object_get(coords, "x"))));
    xywh.push_back(
        static_cast<int>(json_number_value(json_object_get(coords, "y"))));
    xywh.push_back(
        static_cast<int>(json_number_value(json_object_get(coords, "w"))));
    xywh.push_back(
        static_cast<int>(json_number_value(json_object_get(coords, "h"))));
